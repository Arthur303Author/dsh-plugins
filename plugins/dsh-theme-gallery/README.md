...
- Applies the selected family through one `overrideTokens()` layer with complete `{ light, dark }` token pairs. Every token name is one the official theme package actually defines, and the official theme service picks the value for the active color scheme — the plugin never resolves light/dark itself.
...
- Adds exactly one DOM node, its own `<style data-theme-gallery>` element in the document head, and removes it when the plugin stops. It writes no page body attributes, does not change the document title, adds no icons, and never reads or modifies the host's DOM.
...
1. Open **Settings**.
2. Open **General**.
3. Under **精选外观 / Curated appearance**, pick one of the 15 curated source-neutral token families. Cards are searchable by Chinese name, English name or family id.
4. Use DSH's built-in **Appearance** control to choose Light, Dark, or Follow system.

The gallery keeps only the selected family in `localStorage`; everything else about appearance stays
with DSH.

## Attribution

The 15 source-neutral theme families are MIT-licensed original palette adaptations with no upstream
syntax definitions to credit.

## License

The package (theme catalog, client UI, build) is **MIT** — see [`LICENSE`](./LICENSE).
