...
1. 打开 DSH 设置；
2. 进入“通用”；
3. 在“精选外观”下选择一个主题家族（支持按中文名、英文名或家族 id 搜索）；
4. 在 DSH 原生“外观”设置中选择浅色、深色或跟随系统。

插件只在本地记住选中的主题家族，其余外观行为全部交还 DSH。
...
插件在页面上只新增一个属于它自己的 `<style data-theme-gallery>` 元素（插件停止时移除），其余外观全部通过官方 `overrideTokens()` 生效。它不写页面 body 属性、不改文档标题、不加图标，也不读取或修改宿主与其它插件的 DOM。
...
DSH 会根据原生外观设置自动选择当前应使用的值。插件只保存用户选择的主题家族，不保存、也不覆盖 DSH 的外观模式。用到的 token 名全部是官方主题包中真实存在的名字（共 24 个），因此每一次覆盖都会生效。
...
本插件为 **MIT** 许可，15 个主题家族均为原创、来源中立的配色改编，无需向上游语法定义致谢。完整许可全文见 [`LICENSE`](./LICENSE)。
